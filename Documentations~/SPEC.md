...
#### POST `/game-view/recorder/settings`
Recorder の設定を更新する。Unity Recorder パッケージ（`com.unity.recorder`）が必要。設定はドメインリロードまでメモリ上に保持される。
...
#### GET `/game-view/recorder/settings`
現在の Recorder 設定を取得する。

レスポンス: POST `/game-view/recorder/settings` と同じ構造の JSON

#### POST `/game-view/recorder/start`
録画を開始する。Play Mode 専用。事前に設定した Recorder 設定を使用する。
...
```
- `fps`: 任意。デフォルト 30
- `frameRatePlayback`: `"Constant"`, `"Variable"`。デフォルト `"Constant"`
- `recordMode`: `"Manual"`, `"SingleFrame"`, `"FrameInterval"`, `"TimeInterval"`。デフォルト `"Manual"`
- `startTime`/`endTime`: TimeInterval 時の開始・終了時間（秒）
- `startFrame`/`endFrame`: FrameInterval 時の開始・終了フレーム
- `frameNumber`: SingleFrame 時のフレーム番号

レスポンス: `{"success": true}`

#### POST `/game-view/recorder/stop`
録画を停止し、ファイルを書き出す。
