...
#### GET `/custom-tools/manifest`

Unity プロジェクト側で定義されたカスタムツールのメタデータを返す。MCP サーバーと CLI はこの manifest を読んで動的にカスタムツール/コマンドを公開する。
...
```json
{
  "tools": [
    {
      "name": "spawn_npc",
      "description": "Spawn NPC placeholders in the active scene.",
...
- `parameters` は配列で表現する（Unity の `JsonUtility` が辞書を扱えないため）
- サポート型は `string` / `boolean` / `integer` / `number` と、それらの配列または `List<T>`
- `required` は `[UniCortexRequired]` 属性で指定する

#### POST `/custom-tools/invoke`

manifest に含まれるカスタムツールを名前で実行する共通 endpoint。`argumentsJson` はカスタムツール引数全体を表す JSON 文字列。

リクエストボディ:
```json
{
  "toolName": "spawn_npc",
  "argumentsJson": "{\"npcName\":\"Guard\",\"count\":3}"
}
```
...
```json
{
  "content": "Spawned 3 NPCs for Guard."
}
```

- 引数が不要なツールでは `argumentsJson` は `"{}"` を送る
- 実際の Unity API 呼び出しはメインスレッド dispatch 経由で行う
- カスタム HTTP ルートは `IUniCortexCustomRoute` で別途直接登録されるため、この manifest には含まれない
...
利用者は Unity プロジェクト側の Editor C# だけで、UniCortex 本体を変更せずにカスタム MCP ツール・CLI コマンド・HTTP ルートを追加できる。

- `IUniCortexCustomTool` / `UniCortexCustomToolBase<TArguments>`:
  - Unity プロジェクト側のカスタムツール定義
  - `Definition` で `name` / `description` / `cliCommand` / `exposeToMcp` / `exposeToCli` を宣言
  - 引数 DTO は public field ベースで、`[UniCortexRequired]` 属性で必須指定
- `IUniCortexCustomRoute`:
  - Unity プロジェクト側のカスタム HTTP ルート定義
  - `UniCortexCustomRouteDefinition` で method/path を宣言し、`Handle` で JSON レスポンスを返す
- `CustomExtensionRegistry`:
  - Editor 起動時に利用側アセンブリを走査し、カスタムツール/ルートを発見
  - ルート重複を拒否し、manifest を構築
- `GET /custom-tools/manifest` + `POST /custom-tools/invoke`:
  - MCP と CLI の runtime proxy が共有する bridge endpoint
- CLI の custom command:
  - manifest-driven の pre-dispatch で解決
  - named option のみ対応 (`--option value` / `--option=value`)
  - bool は bare flag 可、配列は同じ option を繰り返して渡す
  - built-in top-level group (`editor`, `scene`, `gameobject`, `custom`, `timeline` など) との衝突は禁止
- MCP custom tool:
  - 起動時に manifest を読み込み、組み込み tool に加えて `tools/list` / `tools/call` に合流させる
  - manifest を再読込するには MCP クライアント側でサーバー再起動が必要
...
- **ユースケース層**: `EditorUseCase`, `GameObjectUseCase`, `ComponentUseCase`, `SceneUseCase`, `PrefabUseCase`, `TestUseCase`, `ConsoleUseCase`, `AssetUseCase`, `MenuItemUseCase`, `CustomToolUseCase`, `ScreenshotUseCase`, `SceneViewUseCase`, `GameViewUseCase`, `InputUseCase`, `TimelineUseCase`
...
組み込み MCP ツールと runtime-discovered なカスタム MCP ツールを公開する薄いラッパー。組み込みツールクラスは対応する Core ユースケースをコンストラクタ DI で受け取り、結果を `CallToolResult` でラップする。
...
- `.WithToolsFromAssembly()` で組み込みツール自動検出
- bootstrap 用 ServiceProvider で `CustomToolUseCase` を作り、Unity 側 manifest から custom MCP tool metadata を取得
- custom tool が存在する場合は `.WithListToolsHandler()` / `.WithCallToolHandler()` で runtime registration
...
### MCP ツール（組み込み + runtime-discovered custom tools）

組み込みツールは `[McpServerToolType]` クラス内に `[McpServerTool]` メソッドとして定義する。これに加えて、Unity 側 manifest に含まれる custom tool を起動時に読み込み、同じ `tools/list` / `tools/call` surface に追加する。
...
- custom command は manifest-driven であり、位置引数ではなく名前付きオプションのみを受け付ける

### エントリポイント（Program.cs）

- `ConsoleApp.Create()` で CLI アプリを構築
- `.ConfigureServices()` で `AddUniCortexCore()` を呼び出し DI 登録
- built-in top-level command に一致しない場合は先に custom command pre-dispatch を試行
- `app.Add<XxxCommands>("prefix")` でコマンドグループを登録
- 各コマンドクラスは対応する Core サービスをコンストラクタ DI で受け取る

### カスタム CLI コマンド

- `custom list` で Unity プロジェクトから公開されている custom command 一覧を取得できる
- custom command は `GET /custom-tools/manifest` を元に longest-prefix match で解決する
- 実行時は `POST /custom-tools/invoke` に転送し、Unity プロジェクト側のカスタムツール実装へ委譲する
- オプション形式:
  - `--option value`
  - `--option=value`
  - bool は `--flag` または `--flag true/false`
  - 配列は同じ option を繰り返す
