...
#### POST `/game-view/size`
Game View の解像度を設定する。`index` 指定（リストから選択、推奨）と `width`/`height` 指定（カスタム追加）の両方に対応。`index >= 0` の場合は `index` が優先される。
...
| `set_game_view_size` | POST `/game-view/size` | Game View の解像度を設定（インデックスまたは幅・高さ） |
...
screenshot capture
scene-view focus
game-view focus|size|size-list|set-size|set-size-index
