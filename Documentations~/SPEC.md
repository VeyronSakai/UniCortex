...
ユーザーが定義したカスタム MCP ツールの一覧取得と実行。`CustomToolHandler` を継承したクラスを Unity Editor 側で実装すると、自動的に発見・登録される。

#### GET `/custom-tools/list`
登録済みのカスタムツール一覧を返す。`TypeCache.GetTypesDerivedFrom<CustomToolHandler>()` で発見されたすべてのカスタムツールのメタデータを返す。
...
- `inputSchema`: JSON Schema 文字列。`CustomToolSchema` から自動生成される。パラメータなしのツールは `null`

#### POST `/custom-tools/execute`
カスタムツールを名前指定で実行する。`CustomToolHandler.Execute()` はメインスレッドで実行されるため、Unity API の呼び出しが安全に行える。
...
MCP サーバー起動時に Unity Editor の `GET /custom-tools/list` から動的に発見される。`WithListToolsHandler` / `WithCallToolHandler` で既存の静的ツールと統合される。

ユーザーは `CustomToolHandler` を継承したクラスを Unity Editor 側に実装し、`CustomToolSchema` で入力パラメータを C# で定義する。
