...
Unity Recorder パッケージ（`com.unity.recorder`）を使用した録画機能。複数の Recorder をリストで管理し、一括で録画開始・停止を行う。Recorder のリストはドメインリロードでリセットされる。

#### POST `/recorder/add`
Movie Recorder をリストに追加する。Source は GameView 固定、Audio ON 固定、解像度は Game View Resolution 固定。
...
#### POST `/recorder/remove`
指定したインデックスの Recorder をリストから削除する。
...
#### POST `/recorder/start`
指定したインデックスの Recorder で録画を開始する。Play Mode 専用。Manual モード・Constant フレームレート固定。
...
```
- `index`: 使用する Recorder のインデックス（`get_recorder_list` で取得）
- `fps`: 任意。デフォルト 30
