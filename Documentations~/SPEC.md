...
Unity 組み込みの Profiler ウィンドウと録画状態を制御する。追加パッケージは不要。

#### POST `/profiler/focus`
Profiler ウィンドウを開く、または既に開いている Profiler ウィンドウへフォーカスを移す。

レスポンス: `{"success": true}`

#### GET `/profiler/status`
Profiler ウィンドウの表示状態と recording 状態を取得する。
...
- `isWindowOpen`: Profiler ウィンドウが開いているか
- `hasFocus`: Profiler ウィンドウがフォーカスを持っているか
- `isRecording`: 現在 recording 中か
- `profileEditor`: `UnityEditorInternal.ProfilerDriver.profileEditor` の現在値。`true` の場合は Unity Editor 自体を対象にする

#### POST `/profiler/recording/start`
Profiler recording を開始する。
...
- `profileEditor`: 任意。`true` の場合は Unity Editor 自体をプロファイル対象にする。デフォルトは `false`

レスポンス: `{"success": true}`

#### POST `/profiler/recording/stop`
Profiler recording を停止する。
...
- **ユースケース層**: `EditorUseCase`, `GameObjectUseCase`, `ComponentUseCase`, `SceneUseCase`, `PrefabUseCase`, `TestUseCase`, `ConsoleUseCase`, `AssetUseCase`, `MenuItemUseCase`, `ScreenshotUseCase`, `MovieRecordingUseCase`, `SceneViewUseCase`, `GameViewUseCase`, `ProfilerUseCase`, `InputUseCase`, `TimelineUseCase`, `ExtensionUseCase`
...
| `create_game_object` | POST `/gameobject/create` | GameObject を作成（プリミティブ指定可） |
| `delete_game_object` | POST `/gameobject/delete` | GameObject を削除 |
| `modify_game_object` | POST `/gameobject/modify` | 名前変更・有効/無効・親子関係・タグ・レイヤーの変更 |
...
| ツール | API | 説明 |
|--------|-----|------|
| `focus_profiler_window` | POST `/profiler/focus` | Profiler ウィンドウを開く、またはフォーカスする |
| `get_profiler_status` | GET `/profiler/status` | Profiler ウィンドウと recording 状態を取得 |
| `start_profiler_recording` | POST `/profiler/recording/start` | Profiler recording を開始。任意で Editor profiling を有効化 |
| `stop_profiler_recording` | POST `/profiler/recording/stop` | Profiler recording を停止 |

#### Recorder（5）

| ツール | API | 説明 |
|--------|-----|------|
| `get_all_recorders` | GET `/recorder/all/list` | 登録済みの Recorder 一覧と設定・エラーを取得（要 com.unity.recorder） |
| `add_movie_recorder` | POST `/recorder/movie/add` | Movie Recorder を追加（要 com.unity.recorder） |
| `remove_movie_recorder` | POST `/recorder/movie/remove` | Movie Recorder を削除（要 com.unity.recorder） |
| `start_movie_recorder` | POST `/recorder/movie/start` | Movie Recorder で録画を開始（要 com.unity.recorder） |
| `stop_movie_recorder` | POST `/recorder/movie/stop` | Movie Recorder の録画を停止（要 com.unity.recorder） |
...
profiler focus|status|start|stop
recorder all list
