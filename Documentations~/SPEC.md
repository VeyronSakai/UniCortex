...
- `Tools~/` — `~` サフィックスにより Unity のインポート対象外。.NET 10 MCP サーバープロジェクト
...
- `nameFilter`: テスト名フィルタ（単一文字列、任意、省略時は全テスト実行）。`testNames` が指定された場合は無視される
- `testNames`: テスト名の配列（任意）。`nameFilter` より優先される
