...
#### POST `/view/focus-scene`
Scene View にフォーカスを切り替える。

レスポンス: `{"success": true}`

#### POST `/view/focus-game`
...
- **ユースケース層**: `EditorUseCase`, `GameObjectUseCase`, `ComponentUseCase`, `SceneUseCase`, `PrefabUseCase`, `TestUseCase`, `ConsoleUseCase`, `AssetUseCase`, `MenuItemUseCase`, `ScreenshotUseCase`, `ViewUseCase`, `InputUseCase`, `TimelineUseCase`
...
| ツール | API | 説明 |
|--------|-----|------|
| `focus_scene_view` | POST `/view/focus-scene` | Scene View にフォーカスを切り替え |
| `focus_game_view` | POST `/view/focus-game` | Game View にフォーカスを切り替え |
