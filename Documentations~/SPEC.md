...
Game View の録画を開始する。Play Mode 専用。Unity Recorder パッケージ（`com.unity.recorder`）が必要。

- `RecorderController` + `MovieRecorderSettings` で Game View を MP4 録画
- `SetRecordModeToManual()` で手動停止まで録画を継続
- `FrameRatePlayback.Constant` + `CapFrameRate = true` で固定フレームレート
...
リクエストボディ:
```json
{"fps": 30, "outputPath": "/path/to/output.mp4"}
```
- `fps`: 任意。デフォルト 30
- `outputPath`: 任意。未指定時はシステム一時ディレクトリに自動生成

レスポンス: `{"success": true}`

#### POST `/game-view/recorder/stop`
録画を停止し、MP4 ファイルを書き出す。
