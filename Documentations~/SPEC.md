...
`EditorWindow.SendEvent()` で Game View にイベントを送信する。Play モード必須。

**制約**: `EditorWindow.SendEvent()` は IMGUI/Editor レベルのイベント。OnGUI および UGUI EventSystem には有効だが、`UnityEngine.Input.GetKey()` / `Input.GetMouseButton()`（OS レベル入力）はトリガーされない。

#### POST `/input/key`
Play モード中に Game View にキーボードイベントを送信する。
...
- `keyName`: 必須。`Event.KeyboardEvent()` のキー名（例: `"space"`, `"return"`, `"a"`, `"up"`）。修飾子: `"#a"`（Shift+A）, `"^a"`（Ctrl+A）, `"%a"`（Alt+A）
- `eventType`: 任意。`"keyDown"`（デフォルト）または `"keyUp"`

レスポンス: `{"success": true}`

#### POST `/input/mouse`
Play モード中に Game View にマウスイベントを送信する。
...
- `x`, `y`: Game View ローカル座標
- `button`: 任意。0=左（デフォルト）, 1=右, 2=中
- `eventType`: 任意。`"mouseDown"`（デフォルト）または `"mouseUp"`

レスポンス: `{"success": true}`

### 入力シミュレーション（Input System）

Unity Input System パッケージ（`com.unity.inputsystem`）を使用したデバイスレベルの入力シミュレーション。`InputState.Change()` でデバイス状態を直接変更する。Play モード必須。Input System パッケージがインストールされている場合のみ利用可能。
...
**オプショナル依存**: `UniCortex.Editor.InputSystem` アセンブリは `com.unity.inputsystem` がインストールされている場合のみコンパイルされる（asmdef の `defineConstraints` で制御）。未インストール時はフォールバックアダプターが `NotSupportedException` を返す。

#### POST `/input/system/key`
...
#### POST `/input/system/mouse`
Play モード中に Input System 経由でマウスイベントを送信する。

リクエストボディ:
```json
{"x": 100.0, "y": 200.0, "button": 0, "eventType": "press"}
```
...
- **ユースケース層**: `EditorUseCase`, `GameObjectUseCase`, `ComponentUseCase`, `SceneUseCase`, `PrefabUseCase`, `TestUseCase`, `ConsoleUseCase`, `AssetUseCase`, `MenuItemUseCase`, `ScreenshotUseCase`, `InputUseCase`
...
| ツール | API | 説明 |
|--------|-----|------|
| `send_key_event` | POST `/input/key` | Play モード中に Game View にキーイベントを送信（IMGUI/UGUI 向け） |
| `send_mouse_event` | POST `/input/mouse` | Play モード中に Game View にマウスイベントを送信（IMGUI/UGUI 向け） |
| `send_input_system_key_event` | POST `/input/system/key` | Input System 経由でキーイベントを送信（要 com.unity.inputsystem） |
| `send_input_system_mouse_event` | POST `/input/system/mouse` | Input System 経由でマウスイベントを送信（要 com.unity.inputsystem） |
...
```
editor ping|play|stop|undo|redo|reload-domain
...
menu execute
screenshot capture
input send-key|send-mouse|send-system-key|send-system-mouse
```
