...
型解決は `componentType`（namespace を含む完全修飾型名）と `assemblyName`（その型を定義するアセンブリ名）の組で行う。内部的には `Type.GetType($"{componentType}, {assemblyName}")` を使用するため、`assemblyName` は CLR のアセンブリ単純名（例: `UnityEngine.PhysicsModule`、`Assembly-CSharp`）を渡す。

#### POST `/component/add`
GameObject にコンポーネントを追加する。`Undo.AddComponent` で Undo 対応。

リクエストボディ: `{"instanceId": 12345, "componentType": "UnityEngine.Rigidbody", "assemblyName": "UnityEngine.PhysicsModule"}`
...
リクエストボディ: `{"instanceId": 12345, "componentType": "UnityEngine.Rigidbody", "assemblyName": "UnityEngine.PhysicsModule", "componentIndex": 0}`
...
- `assemblyName`: その型を定義するアセンブリ名（必須）
...
`.asset` ファイル（ScriptableObject）の新規作成・読み取り・書き込みを行う。型解決はコンポーネントと同様に `typeName` + `assemblyName` の組で行う。プロパティの読み書きは `SerializedPropertyValueConverter` / `SerializedPropertyValueParser` を流用し、`get_component_properties` / `set_component_property` と同じ string ベースのフォーマットを使う。

Read で列挙するのはトップレベルのプロパティのみ。入れ子の `[Serializable]` 型の中身は `Generic` 1 件として現れる。Write は `SerializedObject.FindProperty` を用いるため、`nestedField.x` や `arrayField.Array.data[0].value` のような dotted path 書き込みに対応する。

#### POST `/scriptable-object/create`
完全修飾型名を指定して新規 `.asset` を作成する。`ScriptableObject.CreateInstance` + `AssetDatabase.CreateAsset` + `Undo.RegisterCreatedObjectUndo`。
...
レスポンス: `{"success": true, "instanceId": 56789}`

#### GET `/scriptable-object/properties?assetPath=Assets/Data/MyData.asset`
既存 `.asset` ファイルのトップレベルプロパティ一覧を返す。

クエリパラメータ:
- `assetPath`: 対象 `.asset` ファイルのアセットパス（必須）
...
`.asset` 上の特定プロパティを更新する。`SerializedObject.ApplyModifiedProperties` 経由で自動 Undo + `EditorUtility.SetDirty` + `AssetDatabase.SaveAssets`。
...
| `set_component_property` | POST `/component/property` | コンポーネントのプロパティを変更 |

型は `componentType` + `assemblyName` で指定（例: `UnityEngine.Rigidbody` + `UnityEngine.PhysicsModule`）。

#### ScriptableObject（3）

| ツール | API | 説明 |
|--------|-----|------|
| `create_scriptable_object` | POST `/scriptable-object/create` | `typeName` + `assemblyName` から新規 `.asset` を作成 |
| `get_scriptable_object_properties` | GET `/scriptable-object/properties` | `.asset` ファイルのトップレベルプロパティ一覧を取得 |
| `set_scriptable_object_property` | POST `/scriptable-object/property` | `.asset` ファイルの特定プロパティを変更 |
...
- 専用 ScriptableObject ツール — Component と同じ `SerializedProperty` ベースの語彙で `.asset` を編集できるため、エージェントから一貫した API として扱える。ファイルシステム直接編集ではフォーマット壊れや Undo・Inspector 反映のロストが起きやすい
