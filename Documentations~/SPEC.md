...
- `eventType`: 任意。`"press"`（デフォルト）, `"release"`, または `"move"`（位置のみ更新、ボタン操作なし）
...
**オプショナル依存**: `UniCortex.Editor.asmdef` の `versionDefines` により、`com.unity.timeline` がインストールされている場合に `UNICORTEX_TIMELINE` が定義される。未インストール時はフォールバックアダプターが `NotSupportedException` を返す。

#### POST `/timeline/create`
TimelineAsset を作成し、指定した GameObject の PlayableDirector に割り当てる。PlayableDirector がない場合は自動追加する。

リクエストボディ:
```json
{"instanceId": 12345, "assetPath": "Assets/Timelines/MyTimeline.playable"}
```

- `instanceId`: 必須。Timeline を割り当てる GameObject の instanceId
- `assetPath`: 必須。TimelineAsset の保存先パス

レスポンス:
```json
{"assetPath": "Assets/Timelines/MyTimeline.playable", "instanceId": 12345}
```

#### GET `/timeline/info`
PlayableDirector の Timeline 情報を取得する。トラック、クリップ、バインディング、再生状態を含む。

クエリパラメータ:
- `instanceId`: PlayableDirector を持つ GameObject の instanceId（必須）

レスポンス:
```json
{
  "timelineAssetName": "MyTimeline",
  "duration": 10.0,
  "currentTime": 2.5,
  "isPlaying": false,
  "tracks": [
    {
      ...
      "locked": false,
      "clips": [
        {"name": "IdleClip", "start": 0.0, "duration": 5.0, "end": 5.0}
      ]
    }
  ],
  "bindings": [
    {
      ...
      "sourceType": "AnimationTrack",
      "boundObjectName": "Player",
      "boundObjectInstanceId": 12345
    }
  ]
}
```

#### POST `/timeline/add-track`
...
- **ユースケース層**: `EditorUseCase`, `GameObjectUseCase`, `ComponentUseCase`, `SceneUseCase`, `PrefabUseCase`, `TestUseCase`, `ConsoleUseCase`, `AssetUseCase`, `MenuItemUseCase`, `ScreenshotUseCase`, `InputUseCase`, `TimelineUseCase`
...
| ツール | API | 説明 |
|--------|-----|------|
| `create_timeline` | POST `/timeline/create` | TimelineAsset を作成し PlayableDirector に割り当て（要 com.unity.timeline） |
| `get_timeline_info` | GET `/timeline/info` | PlayableDirector の Timeline 情報を取得（要 com.unity.timeline） |
| `add_timeline_track` | POST `/timeline/add-track` | TimelineAsset にトラックを追加（要 com.unity.timeline） |
| `remove_timeline_track` | POST `/timeline/remove-track` | TimelineAsset からトラックを削除（要 com.unity.timeline） |
| `set_timeline_binding` | POST `/timeline/set-binding` | トラックバインディングを設定（要 com.unity.timeline） |
| `add_timeline_clip` | POST `/timeline/add-clip` | トラックにクリップを追加（要 com.unity.timeline） |
| `remove_timeline_clip` | POST `/timeline/remove-clip` | トラックからクリップを削除（要 com.unity.timeline） |
...
timeline create|get-info|add-track|remove-track|set-binding|add-clip|remove-clip
```
...
dotnet run --project /path/to/UniCortex/Tools~/UniCortex.Cli/ -- screenshot capture --output-path ./screenshot.png
```
