...
#### POST `/scene-view/camera`
Scene View のカメラ姿勢を直接指定して視点を変更する。
...
- `position` と `rotation` は必須
- `size` は省略時に現在の Scene View サイズを維持
- `orthographic` は省略時に現在の投影モードを維持

レスポンス: `{"success": true}`

#### GET `/scene-view/camera`
現在の Scene View カメラ状態を取得する。

- Scene View が開かれていない場合は自動で開いて取得する
...
| `get_scene_view_camera` | GET `/scene-view/camera` | Scene View の現在のカメラ状態を取得 |
| `set_scene_view_camera` | POST `/scene-view/camera` | Scene View のカメラ姿勢を位置・回転クォータニオンで直接指定 |
...
- `set_scene_view_camera` — Scene View の視点をエージェントが再現可能な数値入力で直接制御するために必要
- `get_scene_view_camera` — 数値で制御した Scene View の現在状態を読み戻し、往復可能な camera 操作を実現するために必要
...
menu execute
screenshot capture
scene-view focus|camera get|set
game-view focus
