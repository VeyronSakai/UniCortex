...
#### GET `/project-settings/get?category=Time`
指定カテゴリのシリアライズ済みプロパティを返す。

クエリパラメータ:
- `category`: カテゴリ名（必須）。有効な値は `/project-settings/categories` を参照
...
#### POST `/project-settings/set`
指定カテゴリのシリアライズ済みプロパティを変更する。`SerializedObject` / `SerializedProperty` API を使用し `Undo` に自動記録され、変更後 `AssetDatabase.SaveAssets()` でディスクに永続化する。
...
- `category`: カテゴリ名（必須）
- `propertyPath`: Unity の `SerializedProperty.propertyPath` 形式（必須）
- `value`: 文字列で指定。型は `SerializedProperty.propertyType` から自動判定
...
| ツール | API | 説明 |
|--------|-----|------|
| `get_project_settings` | GET `/project-settings/get` | 指定カテゴリの ProjectSettings プロパティを取得 |
| `set_project_setting` | POST `/project-settings/set` | ProjectSettings のプロパティを変更（Undo 対応） |
| `list_project_settings_categories` | GET `/project-settings/categories` | 編集可能な ProjectSettings カテゴリ名一覧を取得 |
...
project-settings get|set|categories
