...
Game View のスクリーンショットを取得する。Play Mode 専用。

- `Camera.main`（なければ `targetTexture == null` かつ `depth` 最大のカメラ）から直接レンダリング
...
Scene View のスクリーンショットを取得する。

- `SceneView.lastActiveSceneView.camera` からレンダリング
- Edit Mode / Play Mode 両方で動作
- Scene View ウィンドウが開かれていない場合は 500 エラー
...
- `x`, `y`: スクリーン座標（ピクセル単位）。原点 (0, 0) は画面左下。X は右方向、Y は上方向に増加する。値の範囲は Game View の解像度に依存する（例: 800x600 の場合、x: 0–800, y: 0–600）。`Mouse.current.position.ReadValue()` と同じ座標系
...
- **ユースケース層**: `EditorUseCase`, `GameObjectUseCase`, `ComponentUseCase`, `SceneUseCase`, `PrefabUseCase`, `TestUseCase`, `ConsoleUseCase`, `AssetUseCase`, `MenuItemUseCase`, `GameViewUseCase`, `SceneViewUseCase`, `InputUseCase`, `TimelineUseCase`
...
| ツール | API | 説明 |
|--------|-----|------|
| `capture_game_view` | GET `/game-view/capture` | Game View のスクリーンショットを取得 |
| `capture_scene_view` | GET `/scene-view/capture` | Scene View のスクリーンショットを取得 |
...
- `capture_game_view` / `capture_scene_view` — マルチモーダル AI エージェントが視覚的に状態を確認するために必要
