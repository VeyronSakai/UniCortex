...
#### POST `/prefab/save`
Prefab Mode で開いている Prefab を保存する。`PrefabUtility.SaveAsPrefabAsset()`
...
- `ScreenCapture.CaptureScreenshotAsTexture()` で Game View 全体（UI オーバーレイ含む）をキャプチャ
...
- `x`, `y`: スクリーン座標（ピクセル単位）。原点 (0, 0) は画面左下。X は右方向、Y は上方向に増加する。値の範囲は Game View の解像度に依存する（例: 800x600 の場合、x: 0–800, y: 0–600）。`Mouse.current.position.ReadValue()` と同じ座標系。注意: `capture_game_view` のスクリーンショットは左上原点・Y 下方向増加のため、座標変換が必要
...
- **ユースケース層**: `EditorUseCase`, `GameObjectUseCase`, `ComponentUseCase`, `SceneUseCase`, `PrefabUseCase`, `TestUseCase`, `ConsoleUseCase`, `AssetUseCase`, `MenuItemUseCase`, `GameViewUseCase`, `InputUseCase`, `TimelineUseCase`
...
| `save_prefab` | POST `/prefab/save` | Prefab Mode で開いている Prefab を保存 |
...
- `capture_game_view` — マルチモーダル AI エージェントが視覚的に状態を確認するために必要
