...
Unity Package Manager の標準操作を実行する。`Client` 操作は Unity 側で逐次実行し、非同期リクエストは `EditorApplication.update` で完了を待つ。

#### GET `/package-manager/list`
プロジェクトの解決済みパッケージ一覧を取得する。

クエリパラメータ:
- `offlineMode`: 任意。デフォルト `true`
- `includeIndirectDependencies`: 任意。デフォルト `false`
  ...
  "packages": [
    {
      ...
      "resolvedPath": "/path/to/Library/PackageCache/com.unity.timeline@1.8.7",
      "assetPath": "Packages/com.unity.timeline",
      "description": "Timeline authoring tools",
      "dependencies": [],
      "errors": []
    }
  ]
...
#### GET `/package-manager/search`
Unity package registry から指定パッケージを検索する。

クエリパラメータ:
- `packageIdOrName`: 必須。パッケージ名または package ID
- `offlineMode`: 任意。デフォルト `false`

レスポンス: `/package-manager/list` と同じ `{"packages": [...]}` 形式。

#### POST `/package-manager/add`
パッケージ依存を追加する。`identifier` は Unity の `PackageManager.Client.Add(string identifier)` にそのまま渡す。

リクエストボディ:
```json
{"identifier": "com.unity.timeline@1.8.7"}
```

レスポンス: 追加された `PackageEntry`。

#### POST `/package-manager/remove`
直接依存のパッケージを削除する。対象が直接依存でない、または既に削除済みの場合は成功扱いにする。
...
#### POST `/package-manager/resolve`
Package Manager の解決処理を強制実行する。
...
| ツール | API | 説明 |
|--------|-----|------|
| `list_unity_packages` | GET `/package-manager/list` | プロジェクトのパッケージ一覧を取得 |
| `search_unity_package` | GET `/package-manager/search` | package name / ID で registry を検索 |
| `add_unity_package` | POST `/package-manager/add` | Package Manager identifier でパッケージ依存を追加 |
| `remove_unity_package` | POST `/package-manager/remove` | 直接依存パッケージを削除 |
| `resolve_unity_packages` | POST `/package-manager/resolve` | Package Manager の解決処理を強制実行 |
...
console logs|clear
asset refresh
package list|search|add|remove|resolve
