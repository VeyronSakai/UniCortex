...
Unity Recorder パッケージ（`com.unity.recorder`）を使用した録画機能。複数の Movie Recorder をリストで管理し、一括で録画開始・停止を行う。Movie Recorder のリストはドメインリロードでリセットされる。

#### POST `/movie-recorder/add`
