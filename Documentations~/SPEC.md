...
│   ├── UniCortex.Core/          ← 共有ライブラリ（サービス層 + HTTP インフラ）
│   │   ├── Domains/
│   │   ├── Extensions/
│   │   ├── Infrastructures/
│   │   ├── Services/            ← 10 サービスクラス
...
MCP サーバーと CLI が共有するサービス層・HTTP インフラストラクチャ。

- **サービス層**: `EditorService`, `GameObjectService`, `ComponentService`, `SceneService`, `PrefabService`, `TestService`, `ConsoleService`, `AssetService`, `MenuItemService`, `ScreenshotService`
- **インフラ**: `HttpRequestHandler`, `UnityServerUrlProvider`, `DomainReloadUseCase`, `HttpResponseMessageExtensions`
- **DI 拡張**: `ServiceCollectionExtensions.AddUniCortexCore()` で全サービス・インフラを一括登録

各サービスは `IHttpClientFactory` と `IUnityServerUrlProvider` をコンストラクタ DI で受け取り、Unity Editor HTTP サーバーと通信する。戻り値は `string`（JSON またはメッセージ）または `byte[]`（スクリーンショット）。例外はそのまま呼び出し元に伝播する。

### MCP サーバー（UniCortex.Mcp）

MCP ツール定義のみを担当する薄いラッパー。各ツールクラスは対応する Core サービスをコンストラクタ DI で受け取り、結果を `CallToolResult` でラップする。
